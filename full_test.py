...
def cal_test_error(test_file_num, BATCH_SIZE):
    ...
    if len(test_input_iter) != len(test_output_iter):
        return "different number of test picture"
...
# change : dir_input, dir_test_input(if you want to test all image levels), namelist_input(if you want to test other image levels)
dir_input = '/home/powergkrry/lv_challenge/data/dataset/poriginal_train_png/'
dir_output = '/home/powergkrry/lv_challenge/data/dataset/pcontour_train_png/'
dir_test_input = '/home/powergkrry/lv_challenge/data/dataset/poriginal_onlyoriginal_test_png/original/'
dir_test_output = '/home/powergkrry/lv_challenge/data/dataset/pcontour_test_png/'
namelist_input = os.listdir('/home/powergkrry/lv_challenge/data/dataset/poriginal_train_png/original/folder/') # only original
namelist_output = os.listdir('/home/powergkrry/lv_challenge/data/dataset/pcontour_train_png/folder/')
namelist_test = os.listdir('/home/powergkrry/lv_challenge/data/dataset/poriginal_test_png/original/folder')
namelist_input.sort()
...
view_data_in = img_to_tensorVariable(
    os.path.join(dir_input, 'original/folder/'), namelist_input, 1, N_TEST_IMG
)
...
for dir_input_location in os.listdir(dir_input):
    ...
    folder_input = folder2.ImageFolder(
        root=os.path.join(dir_input, dir_input_location),
    ...
    for epoch in range(EPOCH):
        ...
        if len(train_input_iter) != len(train_output_iter):
            print("different number of train picture")
            break
        ...
        for step in range(len(train_input_iter)):
            ...
            #        print("Epoch :", epoch_, "| step :",step,"| train loss: %0.6f" % loss.data[0])
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            for i in range(N_TEST_IMG):
                a[2][i].clear()
                a[2][i].imshow(decoded_data.data.cpu().numpy()[i].reshape(256,256), cmap='gray')
                a[2][i].set_xticks(())
                a[2][i].set_yticks(())
            plt.draw()
            plt.pause(0.05)
